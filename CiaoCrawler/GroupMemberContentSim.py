...
GCS_file = open('Final_GMCS.txt','w')
...
for each_group in grp_grt_2:
    ...
    #for prod in common_prods:
        ...
        #for user in each_group:
            ...
            x = x.lower()
            words = re.findall(r'\w+', x,flags=re.UNICODE | re.LOCALE)
            #print(words)
            important_words = ' '.join(filter(lambda y: y not in cachedWords, words))
            x = important_words
